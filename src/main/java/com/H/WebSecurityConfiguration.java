...
import com.H.Modle.RestAuthenticationFailureHandler;
import com.H.Modle.RestAuthenticationSuccessHandler;
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.formLogin() // 登陆的设置
                ...
                //.loginPage("loginPage")  //使用的自定义登录页面
                .loginProcessingUrl("/login")//登录请求的url  default is /login with an HTTP post
                //.defaultSuccessUrl("/user/add")//登陆成功后默认的登录跳转页面
                .permitAll(); // premit all authority登录页面允许所有人访问
        http.authorizeRequests().antMatchers("/user/*").hasAnyAuthority("USER");//拦截指定url,放行指定角色
        //http.authorizeRequests().antMatchers("/log").permitAll();//允许哪些页面对所有人开放
        }
